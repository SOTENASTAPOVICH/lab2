...
namespace Lab1
{
    public class Register
    {
        ...
        {
            bool result = password.Equals(password2);

            if (result)
                return ("True", "OK");
            else
                return ("False", "Заданные пароли не совпадают");
        }

        private static (string, string) CheckPasswordContent(string password)
        {
            bool reCommonPasswd = Regex.IsMatch(password, @"^[А-Яа-я0-9!@#\$%\^&\*\(\)_\+`~\-=\[\]\{\}\|\\:\;"",\.<>\?\/]+$");
            bool reUpLetter = Regex.IsMatch(password, @"[А-Я]");
            bool reDownLetter = Regex.IsMatch(password, @"[а-я]");
            bool reDigit = Regex.IsMatch(password, @"\d+");
            bool reSpecialSymbol = Regex.IsMatch(password, @"[!@#\$%\^&\*\(\)_\+`~\-=\[\]\{\}\|\\:\;"",\.<>\?\/]");
